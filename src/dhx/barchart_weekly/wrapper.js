...
// hides headerEl entirely whenever BOTH keys are absent AND showToolbar is falsy, so this header
// stays fully inert (no blank row, no reserved space) on page 50692 - only page 50708 (which now
// sends both keys plus showToolbar - see that page's RefreshChart) ever shows it.
//
...
// replacing the BC action bar (RefreshAction/PreviousAction/TodayAction/NextAction) page 50708
// "Requested vs Capacity Weekly P" used to expose via its own actions() area, now retired: BC
...
// file is SHARED with page 50692 "Requested vs Capacity Weekly" (see BuildHeader's own comment),
...
// 50692 - only page 50708 (which now sends 'showToolbar': true - see that page's RefreshChart) ever
...
// call/try-catch pattern as the contextmenu "Show Data" handler in BOOT below. Page 50708's own
// usercontrol() trigger bodies for these events call the exact same local procedures
// (RefreshData/RefreshPeriod with PeriodStartDate -+ 7) its now-removed BC actions used to call -
...
// check - see BuildToolbar's own comment), so this stays a no-op on page 50692, which never sends
// any of these keys.
