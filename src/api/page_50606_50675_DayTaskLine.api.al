page 50675 "Daytask Line Opt"
{
    // Dedicated PageType = API subpage for Daytask Lines.
    ...
    begin
        ...
        then
            Error('You must specify a Job No. and Job Task No. filter to access day task lines.');
        GlobalSessionVar.ResetDayTaskTemp();
    end;

    trigger OnInsertRecord(BelowxRec: Boolean): Boolean
    var
        TempLine: Record "Day Tasks" temporary;
        ExistingLine: Record "Day Tasks";
        NextLineNo: Integer;
        GuidVar: Guid;
        ModifyRec: Boolean;
    begin
        ...
        GlobalSessionVar.SetDayTaskTemp(Rec); // stash the inserted/updated line in a global temp table for retrieval by the parent header API
