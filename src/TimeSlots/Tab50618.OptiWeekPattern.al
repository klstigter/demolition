table 50618 "Opti Week Pattern"
{
    ...
    procedure UpdateTotals(TotalMinutes: Integer; NumberOfTimeSlots: Integer)
    begin
    ...
    begin
        ...
        CapacityPatternDay.SetRange(
            "Capacity Pattern ID",
            "Week Pattern ID");
        CapacityPatternDay.DeleteAll(true);
    ...
    var
        DayPattern: Record "Opti Day Pattern";
    ...
    begin
        ...
        DayPattern.SetRange("Pattern Hash", DayPatternHash);
        if DayPattern.FindFirst() then
            exit(DayPattern."Day Pattern ID");

        DayPattern.Init();
        DayPattern.Description :=
            CopyStr(
                StrSubstNo(
                    GeneratedDayPatternDescriptionLbl,
                    CopyStr(DayPatternHash, 1, 8)),
                1,
                MaxStrLen(DayPattern.Description));

        DayPattern.Insert(true);
        ...
        if TempTimeSlotBuffer.FindSet() then
            repeat
                DayPatternLine.Init();
                DayPatternLine."Day Pattern ID" := DayPattern."Day Pattern ID";
                DayPatternLine."Time Slot ID" := TempTimeSlotBuffer."Time Slot ID";
    ...
    local procedure InsertCapacityPatternDay(
        WeekdayNo: Integer;
        DayPatternID: Integer)
    var
        CapacityPatternDay: Record "Opti Week Pattern Line";
    ...
    begin
        ...
        CapacityPatternDay.Init();
        CapacityPatternDay."Capacity Pattern ID" :=
            "Week Pattern ID";

        CapacityPatternDay.Validate(
            "Weekday No.",
            WeekdayNo);
        ...
        CapacityPatternDay.Insert(true);
    end;
